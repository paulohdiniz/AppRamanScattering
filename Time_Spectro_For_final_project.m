classdef Time_Spectro_For_final_project < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        Time_average_2                  matlab.ui.control.UIAxes
        ...
        HyperspectralsImagesTab         matlab.ui.container.Tab
    ...
    properties (Access = public)
        ...
        waveform_generators
        liste_UI_axes
        hp_channels %Contient les raw datas de toutes les channels 
        wavename_DZ1
        wavename_DZ2
        frame_is_done=0
        waves_DZ
    ...
    methods (Access = private,Static)
        ...
        function initialise_wave_gen(app)
            ...
            app.resource_store =  evalin('base','dabs.resources.ResourceStore()');
            app.waveform_generators = app.resource_store.filterByClass('dabs.generic.WaveformGenerator');
        ...
        function updateChannels(app)
            ...
            if ((app.hSI.hStackManager.framesDone==0) && (strcmp(app.hSI.acqState,'grab'))) || (strcmp(app.hSI.acqState,'focus'))
                ...
                clear app.trigger_data app.raman_data app.transmission_data app.data_cube app.T_datacube
                ...
                try
                    ...
                    for ii = 1:size(app.liste_UI_axes,2)    % loop on the axis on
                        if ~strcmp(cell_channels{ii}, 'Off')
                            ...
                            imagesc(app.liste_UI_axes(ii), (app.hSI.hDisplay.lastFrame{indice}));%\ dans scanimage, les raw datas se trouvent dans l'objet hSI.hDisplay.lastFrame{}
                            xlim(app.liste_UI_axes(ii),[1 size((app.hSI.hDisplay.lastFrame{indice}),2)])
                            ylim(app.liste_UI_axes(ii),[1 size((app.hSI.hDisplay.lastFrame{indice}),1)])
                ...
                if any(cellfun(@(x) strcmp(x,'Raman Active'),{{app.channel_state(app.ChannelDropDown.Value)},{(app.ChannelDropDown_2.Value)},{app.ChannelDropDown_3.Value}}))
                    ...
                    app.T_datacube=zeros(app.NyEditField_2.Value,app.NxEditField_2.Value,app.NtEditField_2.Value);
                    try
                        ...
                        tempT=squeeze(mean(app.T_datacube,[1 2]));
        ...
        function process_data(app)
            ...
            switch app.raman_average_state
                case 1
                    ...
                    temp_T_datacube=zeros(app.NyEditField_2.Value*app.NxEditField_2.Value,app.NtEditField_2.Value);
                    ...
                    for jj=1:length(I)-1
                        ...
                        temp_T_datacube(jj,:)=tempT((I(jj)-shift):(I(jj)+app.NtEditField_2.Value-shift-1));
                    end
                    temp_data_cube(app.NyEditField_2.Value*app.NxEditField_2.Value,:)=zeros(1,app.NtEditField_2.Value);
                    temp_T_datacube(app.NyEditField_2.Value*app.NxEditField_2.Value,:)=zeros(1,app.NtEditField_2.Value);
                    ...
                    app.T_datacube=reshape(temp_T_datacube,[app.hSI.hRoiManager.pixelsPerLine/app.NtEditField_2.Value app.hSI.hRoiManager.linesPerFrame app.NtEditField_2.Value]);
                otherwise
            end
        end
    ...
    methods (Access = private)
        ...
        function ROIButtonPushed(app, event)
            figure('Name','Select the desired pixel polygon:');
            imagesc()            %Image of transmission
            colormap('hot')
            mask = roipoly; %mask made by user
            SP = SP.make_raman_spectrum_with_mask(mask); %make ramanspectrum with mask
            tempmask1= SP.ramanSpectrumWithMask;
            plot(axisRamSpec, SP.wn,tempmask1, "-", 'DisplayName','ROI Spectrum','Color', '#006400'); %make Raman Spec of ROI
        end
    end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create Time_average_2
            app.Time_average_2 = uiaxes(app.ImagingTab);
            xlabel(app.Time_average_2, 'X')
            ylabel(app.Time_average_2, 'Y')
            zlabel(app.Time_average_2, 'Z')
            app.Time_average_2.Position = [874 493 375 179];
            ...
            app.InitialParametersPanel.Position = [936 412 267 319];
            ...
            app.ScanzoomfactorEditField.HorizontalAlignment = 'center';
            ...
            app.xyIncrementEditField.HorizontalAlignment = 'center';
            ...
            app.xstepspinnerEditField.HorizontalAlignment = 'center';
            ...
            app.ystepspinnerEditField.HorizontalAlignment = 'center';
            ...
            app.DazzlerTriggerFreqkHzEditField.HorizontalAlignment = 'center';
            ...
            app.ChannelsDisplayListBoxLabel.Position = [10 91 98 22];
            ...
            app.ChannelsDisplayListBox.Position = [35 9 48 76];
            ...
            % Create UIAxes4
            app.UIAxes4 = uiaxes(app.SpectrogramTab);
            title(app.UIAxes4, 'pspectrum')
            xlabel(app.UIAxes4, 'Time(ps)')
            ylabel(app.UIAxes4, 'Frequency(THz)')
            zlabel(app.UIAxes4, 'Z')
            app.UIAxes4.Position = [98 584 535 230];

            % Create UIAxes5
            app.UIAxes5 = uiaxes(app.SpectrogramTab);
            title(app.UIAxes5, 'Persistence spectrum')
            xlabel(app.UIAxes5, 'Frequency(THz)')
            ylabel(app.UIAxes5, 'Power Spectrum (dB)')
            zlabel(app.UIAxes5, 'Z')
            app.UIAxes5.Position = [752 601 715 213];

            % Create UIAxes6
            app.UIAxes6 = uiaxes(app.SpectrogramTab);
            title(app.UIAxes6, 'Spectrogram')
            xlabel(app.UIAxes6, 'X')
            ylabel(app.UIAxes6, 'Y')
            zlabel(app.UIAxes6, 'Z')
            app.UIAxes6.Position = [98 39 541 458];

            % Create UIAxes7
            app.UIAxes7 = uiaxes(app.SpectrogramTab);
            title(app.UIAxes7, 'Power spectrum')
            xlabel(app.UIAxes7, 'X')
            ylabel(app.UIAxes7, 'Y')
            zlabel(app.UIAxes7, 'Z')
            app.UIAxes7.Position = [752 39 734 457];

            % Create HyperspectralsImagesTab
            app.HyperspectralsImagesTab = uitab(app.TabGroup);
            app.HyperspectralsImagesTab.AutoResizeChildren = 'off';
            app.HyperspectralsImagesTab.Title = 'Hyperspectrals Images';

            % Create UIAxes
            app.UIAxes = uiaxes(app.HyperspectralsImagesTab);
            title(app.UIAxes, 'Integrated temporal response')
            xlabel(app.UIAxes, 'delay [ps]')
            ylabel(app.UIAxes, 'Raman oscillations')
            zlabel(app.UIAxes, 'Z')
            app.UIAxes.Position = [54 575 624 234];

            % Create UIAxes_5
            app.UIAxes_5 = uiaxes(app.HyperspectralsImagesTab);
            title(app.UIAxes_5, 'Integrated Raman Spectrum')
            xlabel(app.UIAxes_5, 'X')
            ylabel(app.UIAxes_5, 'Y')
            zlabel(app.UIAxes_5, 'Z')
            app.UIAxes_5.Position = [58 233 624 234];

            % Create UIAxes2
            app.UIAxes2 = uiaxes(app.HyperspectralsImagesTab);
            title(app.UIAxes2, 'Title')
            xlabel(app.UIAxes2, 'X')
            ylabel(app.UIAxes2, 'Y')
            zlabel(app.UIAxes2, 'Z')
            app.UIAxes2.Position = [1014 571 256 237];

            % Create UIAxes2_2
            app.UIAxes2_2 = uiaxes(app.HyperspectralsImagesTab);
            title(app.UIAxes2_2, 'Title')
            xlabel(app.UIAxes2_2, 'X')
            ylabel(app.UIAxes2_2, 'Y')
            zlabel(app.UIAxes2_2, 'Z')
            app.UIAxes2_2.Position = [1014 294 256 237];

            % Create UIAxes2_3
            app.UIAxes2_3 = uiaxes(app.HyperspectralsImagesTab);
            title(app.UIAxes2_3, 'Title')
            xlabel(app.UIAxes2_3, 'X')
            ylabel(app.UIAxes2_3, 'Y')
            zlabel(app.UIAxes2_3, 'Z')
            app.UIAxes2_3.Position = [1014 24 256 237];

            % Create WindowDropDownLabel
            app.WindowDropDownLabel = uilabel(app.HyperspectralsImagesTab);
            app.WindowDropDownLabel.HorizontalAlignment = 'right';
            app.WindowDropDownLabel.Position = [689 786 48 22];
            app.WindowDropDownLabel.Text = 'Window';

            % Create WindowDropDown
            app.WindowDropDown = uidropdown(app.HyperspectralsImagesTab);
            app.WindowDropDown.Items = {'barthannwin', 'bartlett', 'blackman', 'blackmanharris', 'bohmanwin', 'chebwin', 'flattopwin gausswin hamming', 'hann', 'kaiser', 'nuttallwin parzenwin rectwin', 'taylorwin tukeywin', 'triang'};
            app.WindowDropDown.Position = [752 786 100 22];
            app.WindowDropDown.Value = 'blackman';
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = Time_Spectro_For_final_project
